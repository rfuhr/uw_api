...
@Service
@AllArgsConstructor
public class VerificaDuplicidadeConfigMensagemFiscalOperInternaQuery {

	EntityManager em;

	@SuppressWarnings("unchecked")
	public void executeSQL(ConfigMensagemFiscalOperInterna entity) {
		List<Tuple> resultTuples = em
				.createNativeQuery(SQLUtils.obterQuery("verificaDuplicidadeConfigMensagemFiscalOperInterna"), Tuple.class)
				.setParameter("mensagemFiscalId", entity.getConfigMensagemFiscal().getMensagemFiscal().getId())
				.setParameter("operacaoInternaId", entity.getOperacaoInterna().getId())
		...
		if (resultTuples != null && resultTuples.size() > 0) {
			...
			String label = "Validação de Duplicidade da Configuração de Mensagem Fiscal - Operação Interna "; 
			validationErrorResponse.getErrors().add(new ValidationError(label,
					"Não é possível incluir a Operação Interna " + entity.getOperacaoInterna().getId() + 
			...
			throw new UnicidadeException(validationErrorResponse,
					"Ocorreu um erro na validação de Duplicidade da Configuração de Mensagem Fiscal - Operação Interna");
		}
	}
}
