...
@Service
@AllArgsConstructor
public class VerificaDuplicidadeConfigMensagemFiscalSituacTribQuery {

	EntityManager em;

	@SuppressWarnings("unchecked")
	public void executeSQL(ConfigMensagemFiscalSituacTrib entity) {
		List<Tuple> resultTuples = em
				.createNativeQuery(SQLUtils.obterQuery("verificaDuplicidadeConfigMensagemFiscalSituacTrib"), Tuple.class)
				.setParameter("mensagemFiscalId", entity.getConfigMensagemFiscal().getMensagemFiscal().getId())
				.setParameter("situacaoTributariaId", entity.getSituacaoTributaria().getId())
		...
		if (resultTuples != null && resultTuples.size() > 0) {
			...
			String label = "Validação de Duplicidade da Configuração de Mensagem Fiscal - Situação Tributária "; 
			validationErrorResponse.getErrors().add(new ValidationError(label,
					"Não é possível incluir a Situação Tributária " + entity.getSituacaoTributaria().getId() + 
			...
			throw new UnicidadeException(validationErrorResponse,
					"Ocorreu um erro na validação de Duplicidade da Configuração de Mensagem Fiscal - Situação Tributária");
		}
	}
}
